...
let mods: ModuleConfig[] = [
  ...
  { id: "def:wayback", name: "wayback_available", kind: "http", enabled: false, builtin: false, default: true, method: "GET", description: "Internet Archive: is a URL snapshotted, and the closest snapshot.", inputLabel: "a URL or domain", url: "https://archive.org/wayback/available?url={input}" },
  { id: "def:keybase", name: "keybase", kind: "http", enabled: false, builtin: false, default: true, method: "GET", description: "Keybase public identity: linked twitter/github/reddit/domain proofs and PGP keys.", inputLabel: "a Keybase username", url: "https://keybase.io/_/api/1.0/user/lookup.json?usernames={input}" },
  { id: "def:maigret", name: "maigret", kind: "command", enabled: false, builtin: false, default: true, description: "Maigret username enumeration across sites. Returns sites with public accounts and URLs.", inputLabel: "a username", command: "maigret {input} --timeout 8 --no-progressbar -J simple" },
  { id: "def:subfinder", name: "subfinder", kind: "command", enabled: false, builtin: false, default: true, description: "subfinder passive subdomain enumeration (works with zero keys).", inputLabel: "a domain", command: "subfinder -d {input} -silent" },
  { id: "def:nuclei", name: "nuclei_exposures", kind: "command", enabled: false, builtin: false, default: true, description: "nuclei exposure/misconfig/tech/ssl/dns templates against an authorized target. No exploit templates.", inputLabel: "an in-scope URL or host", command: "nuclei -u {input} -tags exposure,misconfig,tech,ssl,dns -silent" },
